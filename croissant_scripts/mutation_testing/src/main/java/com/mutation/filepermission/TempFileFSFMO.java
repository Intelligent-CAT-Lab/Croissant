...
public class TempFileFSFMO extends FileStringFMO {
    ...
    public boolean isApplicable() {
        ...
        while (iterator.hasNext()) {
            ...
            if (staticInvokeExpr.getMethod().getSignature().contains(this.methodSignature)) {
                ...
                this.victimTest.setName("b_victim_"+this.victimTest.getName());
            }
    ...
    protected SootMethod setFilePermission(String fileName, Integer mode) {
        ...
        SootMethod test;
        if (mode == 0) {
            test = this.createTestMethod("a_PolluterMutant__" + this.victimTest.getName() + "__");
        } else {
            test = this.createTestMethod("c_DependenceMutant__" + this.victimTest.getName() + "__");
        }
