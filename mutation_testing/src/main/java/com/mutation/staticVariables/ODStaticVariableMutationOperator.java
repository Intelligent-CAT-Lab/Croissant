...
public class ODStaticVariableMutationOperator extends StringInjector {
    ...
    private String createPolluterTest(SootField field, SootField innerField) {
        ...
        String polluterTest = createTest("a_staticVariablePolluterMutant_" + field.getName()) +
    ...
    private String createVictimTest(SootField field, SootField innerField) {
        ...
        String testName = "CroissantMutant_OD_SVD_Victim_" + field.getName();
        String victimTest = createTest(testName) +
    ...
    private  String createstateSetterTest(SootField field, SootField innerField, int stateSetterNumber) {
        String stateSetterTest = createTest("SVD_cleanerMutant_" + field.getName() + "__" + stateSetterNumber) +
            "    double index = " + stateSetterNumber +"; \n" +
            "if (index >= getCleanerCount()) {return;}\n" +
            "   " + field.getName()+"Mutant." + innerField.getName() + " = 10"+ ";\n" +
...
//        String VictmName = this.createVictimTest(this.analyzer.instanceVarWithStatic, this.analyzer.staticField);
//        for (int i = 0; i < 50; i++) {
//            this.createstateSetterTest(this.analyzer.instanceVarWithStatic, this.analyzer.staticField, i);
//        }

        long endTime= System.currentTimeMillis();
        com.framework.OperatorSelector.MUTANT_TIME.put("CroissantMutant_OD_SVD_Victim_fieldClass();", endTime - startTime);
