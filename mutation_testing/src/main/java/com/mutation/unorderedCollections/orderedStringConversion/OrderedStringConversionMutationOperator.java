 ...
 */
    ...
    public class OrderedStringConversionMutationOperator extends SootMutationOperator {
        ...
        public OrderedStringConversionMutationOperator() {
...
//                    }
        ...
        private boolean locateApiCall() {
            ...
            while(iterator.hasNext()) {
                ...
                if (stmt.getInvokeExpr().getMethod().getSignature().equals("<java.util.AbstractCollection: java.lang.String toString()>")){

//                    InvokeExpr in = stmt.getInvokeExpr();
//                    Local ll = Jimple.v().newLocal("methodNameLocalll", RefType.v("java.lang.String"));
...
//                        System.out.println("============="+temp.getType().toString());
                        if (temp.getType().toString().equals("java.util.HashSet")) {
                            ...
                            //this.elementType = getElementType(stmt.getInvokeExpr().getMethod().getSignature());
...
//        SootMethodRef smGetName = Scene.v().getMethod("<java.lang.Class: java.lang.reflect.Method[] getMethods()>").makeRef();
//        VirtualInvokeExpr invoke = soot.jimple.Jimple.v().newVirtualInvokeExpr(local, smGetName);
...
//        args.add(element);
//        args.add(local);
            //args.add(StringConstant.v("public java.lang.Object java.util.HashMap.remove(java.lang.Object)"));
            args.add(local);
            args.add(local2);
...
//
        Local StringOfOriginalHashSet = getStringofHashSet(local, 0,
            "StringOfOriginalHashSet");
        Local StringOfNewHashSet = getStringofHashSet(local, 0, "StringOfNewHashSet");
        Stmt asstStmt = insertAssert(StringOfOriginalHashSet, StringOfNewHashSet);
    }
    private Local getStringofHashSet(Local HashSet, Integer size, String generalName) {
...
//        Method[] methods = map.getClass().getMethods();
        ...
        SootMethod m = c.getMethodByName("toString");
        //System.out.println(m.getSignature());
        SootMethodRef ref = Scene.v().getMethod("<java.util.AbstractCollection: java.lang.String toString()>").makeRef();
        VirtualInvokeExpr invoke = soot.jimple.Jimple.v().newVirtualInvokeExpr(HashSet, ref);
        Local element = Jimple.v().newLocal(generalName + size.toString(), RefType.v("java.lang.String"));
        AssignStmt assignStmt = Jimple.v().newAssignStmt(element, invoke);
        this.getJimpleBody().getLocals().addFirst(element);
        this.InsertStmtEnd(assignStmt);
        return element;
        ...
        //    private SootClass createClass(Integer size, String generalName, Local methodList){
//        SootClass newClass = new SootClass(generalName+size.toString(), Modifier.PUBLIC);
//        newClass.setSuperclass(Scene.v().getSootClass("java.lang.Class"));
//        Scene.v().addClass(newClass);
//        for (Integer i = 1; i <= size; i++){
//            Value index = IntConstant.v(i);
//            Local temp = Jimple.v().newLocal("temp"+i.toString(),RefType.v(this.elementType));
...
//    }
        private Local getFirstMethod(SootClass Class){
            SootMethodRef smGetName = Scene.v().getMethod("<java.lang.Class: java.lang.reflect.Method[] getDeclaredMethods()>").makeRef();
            Local newArrayLocal = Jimple.v().newLocal("Cla",RefType.v(Class));
            this.getJimpleBody().getLocals().addFirst(newArrayLocal);
            VirtualInvokeExpr invoke = soot.jimple.Jimple.v().newVirtualInvokeExpr(newArrayLocal,smGetName);
            Local element = Jimple.v().newLocal("methodNameLocal", RefType.v(this.elementType));
            AssignStmt assignStmt = Jimple.v().newAssignStmt(element, invoke);
            this.getJimpleBody().getLocals().addFirst(element);
            this.InsertStmtEnd(assignStmt);
        ...
        private Local createLists(Local fromLocal, Integer size, String generalName){
            ...
            Local newArrayLocal = Jimple.v().newLocal(generalName+size.toString(),ArrayType.v(RefType.v(this.elementType),size));
        ...
        public void controlflake(SootMethod method, Local local){
            ...
            for (Integer i =1 ; i<= 10; i++) {
                ...
                Local OriginalHashSet = createHashSet(OriginalList, i, "OriginalHashSet");
                Local NewHashSet = createHashSet(OriginalList, i, "NewHashSet");
                Local FirstElementFromOriginalHashSet = getFirstElementFromHashSet(OriginalHashSet, i, "FirstElementFromOriginalHashSet");
...
//        SootClass sc = Scene.v().loadClassAndSupport("C1");
//        Scene.v().loadNecessaryClasses();
            SootClass mainClass = Scene.v().getSootClass("C10");
            Local mm = getFirstMethod(mainClass);
            Stmt asstStmt = insertAssert(mm, mm);

//        Scene.v().getClasses();
//        System.out.println(Scene.v().getClasses());
//        Method[] mm = this.getClass().getDeclaredMethods();
//        List<SootMethod> C1 =mainClass.getMethods();
...
//        method.getActiveBody().getUnits().insertBefore(ifStmt, asstStmt10);
            ...
            //Local NewList = createLists(getMethodsAgain,i,"NewList");
//            Local firstElementFromOriginalList = getFirstElement(OriginalList,i,"firstElementFromOriginalList");
//            Local firstElementFromNewList= getFirstElement(NewList,i,"firstElementFromOriginalList");

//        List<SootMethod> C1 = this.getClass("C1").getMethods();
//        for (SootMethod m:C1) {
//            System.out.println(m.getName());
//        }
...
//        Local firstElementFromOriginalList10 = getFirstElement(OriginalList10,10,"firstElementFromOriginalList");
//        Local firstElementFromNewList10= getFirstElement(NewList10,10,"firstElementFromOriginalList");

            //Stmt asstStmt10 = insertAssert(firstElementFromOriginalList10,firstElementFromNewList10);
...
//        NewArrayExpr arrayExpr = Jimple.v().newNewArrayExpr(RefType.v(this.elementType), IntConstant.v(10));
//        Local arrayLocal10 = Jimple.v().newLocal("c10",ArrayType.v(RefType.v(this.elementType),10 ));
...
//
//        NewArrayExpr arrayExpr2 = Jimple.v().newNewArrayExpr(RefType.v(this.elementType), IntConstant.v(10));
//        Local arrayLocal102 = Jimple.v().newLocal("c10",ArrayType.v(RefType.v(this.elementType),10 ));
